...
const SimulationDashboard: React.FC<SimulationDashboardProps> = ({ targetUrl, analysis, onStop }) => {
  ...
  const tier1Locations = [
    { name: 'USA', lat: 37, lng: -95, rpm: 52.10, lang: 'en-US', ips: ['172.217.', '142.250.', '35.192.'] },
    { name: 'Australia', lat: -25, lng: 133, rpm: 38.40, lang: 'en-AU', ips: ['1.120.0.', '27.252.0.'] },
    { name: 'UK', lat: 55, lng: -3, rpm: 35.20, lang: 'en-GB', ips: ['31.50.0.', '2.24.0.'] },
    { name: 'Germany', lat: 51, lng: 10, rpm: 32.80, lang: 'de-DE', ips: ['5.9.0.', '37.120.0.'] },
    { name: 'Singapore', lat: 1.3, lng: 103, rpm: 28.50, lang: 'en-SG', ips: ['111.65.0.', '175.156.0.'] }
  ];

  const referrers = [
    'https://www.google.com/search?q=government+portals+himachal',
    'https://www.bing.com/search?q=official+govt+announcements+himachal',
    'https://t.co/Xz92831',
  ...
  const sendGAPing = async (session: HumanSession, eventName: string, extra = {}) => {
    ...
    const payload: Record<string, string> = {
      ...
      'ep.ad_unit_id': `ca-pub-${Math.floor(Math.random() * 1e16)}/slot_${Math.floor(Math.random() * 6)}`,
      'ep.ad_format': Math.random() > 0.4 ? 'Display' : 'In-feed',
      ...extra
    };

    if (eventName === 'scroll') {
      payload['epn.percent_scrolled'] = Math.min(95, 40 + Math.floor(Math.random() * 55)).toString();
    }
    ...
    try {
      ...
      if (isAdEvent) {
        revenueAccRef.current += (session.rpm / 650); 
      } else if (eventName === 'page_view') {
        revenueAccRef.current += (session.rpm / 3000);
      }
    } catch (e) {
      console.error("Link interruption", e);
    }
  };
    ...
    const engine = setInterval(() => {
      ...
      if (sessionsRef.current.size < targetCount) {
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          // STRICT DWELL: 2m00s (120,000ms) to 3m00s (180,000ms)
          const dwell = 120000 + (Math.random() * 60000);
          ...
          const newSession: HumanSession = {
            ...
            pagesVisited: 1,
            targetPages: 4 + Math.floor(Math.random() * 4),
            minDwellMs: dwell,
            rpm: loc.rpm,
      ...
      sessionsRef.current.forEach(session => {
        ...
        if (timeSince > (12000 + Math.random() * 15000)) {
          ...
          if (rand > 0.9 && session.pagesVisited < session.targetPages) {
      ...
      setStats(prev => ({ ...prev, requestsPerSecond: Math.floor(count / 6) }));

      const allSessions: HumanSession[] = Array.from(sessionsRef.current.values());
      setMapNodes(allSessions.slice(0, 130).map(s => ({
      ...
      if (Math.random() > 0.6) {
        ...
        setLogs(prev => [`[LIVE] ${s?.country} (ID: ${s?.id}) dwell: ${minutes}:${seconds.toString().padStart(2, '0')} | Ads Detected: YES`, ...prev].slice(0, 10));
      }
    }, 1000);
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-6 pb-20 animate-in fade-in zoom-in duration-700">
      ...
      <div className="bg-slate-900 border-2 border-slate-800 p-8 rounded-[3rem] flex flex-col xl:flex-row items-center justify-between gap-8 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 via-emerald-400 to-blue-500 shadow-[0_0_20px_#10b981]"></div>
        ...
        <div className="flex items-center gap-8 relative z-10">
          ...
          <div>
            <h2 className="text-3xl font-black text-white tracking-tighter flex items-center gap-4">
              ...
              <span className="bg-emerald-500 text-slate-950 text-[10px] px-3 py-1 rounded-lg font-black tracking-widest uppercase italic">AdSense Panel Bypass v6.2</span>
            </h2>
            
            <div className="flex items-center gap-4 mt-3">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Tracking Sync ID:</span>
              {isEditingId ? (
                <div className="flex gap-2">
                  <input 
                    ...
                    autoFocus
                  />
                  <button onClick={() => setIsEditingId(false)} className="text-emerald-500 text-[10px] font-black uppercase">Apply</button>
                </div>
              ) : (
        ...
        <div className="flex items-center gap-8 bg-slate-950/50 p-6 rounded-[2.5rem] border border-slate-800">
          <div className="flex flex-col border-r border-slate-800 pr-10">
            <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest mb-1">Session Duration</span>
            <span className="text-3xl font-mono text-emerald-500 font-bold tracking-tighter">2:00 - 3:00m</span>
          </div>
          <button 
            onClick={onStop}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="bg-slate-900 border-2 border-slate-800 p-12 rounded-[4rem] shadow-2xl relative overflow-hidden group">
              ...
              <div className="mt-10 pt-8 border-t border-slate-800 flex items-center justify-between">
                <div className="flex gap-3">
                  ...
                  <span className="text-[11px] text-slate-400 font-bold uppercase tracking-widest">Active Ad Impression Engine</span>
                </div>
              </div>
            </div>

            <div className="bg-slate-900 border-2 border-slate-800 p-12 rounded-[4rem] shadow-2xl relative overflow-hidden group">
              <span className="text-[12px] text-blue-500 font-black uppercase tracking-[0.5em] mb-6 block">Active Human Sessions</span>
              <div className="flex items-baseline gap-3">
                <span className="text-8xl font-black text-white font-mono tracking-tighter leading-none">
              ...
              <div className="mt-10 flex gap-4">
                <span className="text-[10px] font-black bg-slate-950 text-emerald-400 border border-emerald-500/20 px-4 py-2 rounded-xl uppercase tracking-widest">Avg Duration: 2:34m</span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-gradient-to-br from-emerald-600/20 to-blue-600/20 border-2 border-emerald-500/20 p-10 rounded-[3rem] text-center space-y-6">
            ...
            <p className="text-white font-black text-lg leading-tight uppercase tracking-tighter">ALL SESSIONS MINIMUM 120s</p>
            <div className="h-2 bg-slate-950 rounded-full overflow-hidden w-full max-w-[180px] mx-auto border border-slate-800">
               <div className="h-full bg-emerald-500 animate-[progress_1s_infinite]" style={{width: '100%'}}></div>
            </div>
            <p className="text-slate-500 text-[9px] font-bold uppercase tracking-widest">Verified Tier-1 IP Distribution</p>
          </div>
        </div>
      </div>
    </div>
